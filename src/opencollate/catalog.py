...
_RULE_LIST = (
    ...
    _rule(
        ...
        "Correct the die-pad name or add it to the physical pad inventory.",
    ),
    _rule(
        "OC5002",
    ...
    _rule(
        "OC6001",
        "sdc-object-not-found",
        Severity.ERROR,
        "An SDC query references no object in the elaborated RTL design.",
        "Correct the hierarchical path, select the intended RTL top, or regenerate the SDC.",
    ),
    _rule(
        "OC6002",
        "clock-definition-mismatch",
        Severity.ERROR,
        "Clock definitions disagree across constraint views.",
        "Align the clock period, waveform, source, and generated-clock relationship.",
    ),
    _rule(
        "OC6003",
        "clock-target-role-mismatch",
        Severity.ERROR,
        "A clock is attached to a pin explicitly classified as a non-clock.",
        "Correct the SDC target or the authoritative pin role.",
    ),
    _rule(
        "OC6101",
        "upf-instance-not-found",
        Severity.ERROR,
        "UPF power intent references an instance absent from the elaborated RTL design.",
        "Correct the UPF element path, scope, or selected RTL configuration.",
    ),
    _rule(
        "OC6102",
        "upf-port-not-found",
        Severity.ERROR,
        "UPF power intent references a port or pin absent from the elaborated RTL design.",
        "Correct the UPF port path or add the expected RTL supply/control port.",
    ),
    _rule(
        "OC6103",
        "upf-object-not-found",
        Severity.ERROR,
        "UPF references a power-intent object that is not defined.",
        "Define the supply, domain, state, switch, or strategy object before referencing it.",
    ),
    _rule(
        "OC6104",
        "duplicate-upf-object",
        Severity.ERROR,
        "A UPF view defines the same power-intent object more than once.",
        "Remove the duplicate command or separate mutually exclusive configurations.",
    ),
    _rule(
        "OC6201",
        "ipxact-interface-port-not-found",
        Severity.ERROR,
        "An IP-XACT interface maps to a physical port absent from its component.",
        "Correct the IP-XACT port map or the RTL component interface.",
    ),
    _rule(
        "OC6202",
        "ipxact-interface-port-map-conflict",
        Severity.ERROR,
        "An IP-XACT interface contains an ambiguous or conflicting port map.",
        "Make logical-to-physical port mappings one-to-one unless the bus definition permits it.",
    ),
    _rule(
        "OC6301",
        "register-missing-from-view",
        Severity.ERROR,
        "A hardware or software address-map view is missing a register.",
        "Regenerate the stale address map or correct the register naming configuration.",
    ),
    _rule(
        "OC6302",
        "register-address-mismatch",
        Severity.ERROR,
        "A register has different addresses or offsets across views.",
        "Correct the hardware address map or regenerate the software header.",
    ),
    _rule(
        "OC6303",
        "register-width-mismatch",
        Severity.ERROR,
        "A register has different widths across views.",
        "Align the register size in IP-XACT and generated software collateral.",
    ),
    _rule(
        "OC6304",
        "register-field-missing-from-view",
        Severity.ERROR,
        "A register field is absent from one participating address-map view.",
        "Regenerate the stale header or add the field to the hardware map.",
    ),
    _rule(
        "OC6305",
        "register-field-layout-mismatch",
        Severity.ERROR,
        "A register field has different bit positions or widths across views.",
        "Correct the field offset/mask or regenerate the software header.",
    ),
    _rule(
        "OC6306",
        "register-access-mismatch",
        Severity.ERROR,
        "Register or field access permissions disagree across views.",
        "Align read/write access metadata in the hardware and software views.",
    ),
    _rule(
        "OC6307",
        "duplicate-register-definition",
        Severity.ERROR,
        "A view defines the same register or field more than once.",
        "Remove the duplicate or select one build/configuration variant.",
    ),
    _rule(
        "OC6308",
        "register-field-reset-mismatch",
        Severity.ERROR,
        "A register field has different reset values across views.",
        "Correct the hardware reset value or regenerate the software header.",
    ),
    _rule(
        "OC6309",
        "invalid-register-field-layout",
        Severity.ERROR,
        "Register fields overlap or extend beyond the declared register width.",
        "Correct the field offsets, masks, or register size in the originating view.",
    ),
    _rule(
        "OC6310",
        "ambiguous-register-identity",
        Severity.ERROR,
        "An unscoped register name matches multiple address-map scopes.",
        "Qualify the register identity or generate a scope-specific software header.",
    ),
    _rule(
        "OC6401",
        "def-object-not-found",
        Severity.ERROR,
        "A DEF connection references no object in the elaborated RTL design.",
        "Correct the DEF endpoint, select the intended RTL top, or regenerate "
        "implementation collateral.",
    ),
    _rule(
        "OC6501",
        "connectivity-endpoint-not-found",
        Severity.ERROR,
        "A connectivity requirement endpoint matches no elaborated RTL signal.",
        "Correct the hierarchical endpoint, bit selection, or selected RTL top.",
    ),
    _rule(
        "OC6502",
        "connectivity-endpoint-ambiguous",
        Severity.ERROR,
        "A connectivity requirement endpoint selector matches multiple signals.",
        "Replace the selector with one exact hierarchical signal or bit range.",
    ),
    _rule(
        "OC6503",
        "required-connectivity-missing",
        Severity.ERROR,
        "A required static RTL connectivity path cannot be found.",
        "Correct the RTL connection or the declarative connectivity requirement.",
    ),
    _rule(
        "OC6504",
        "forbidden-connectivity-present",
        Severity.ERROR,
        "A forbidden static RTL connectivity path exists.",
        "Remove the unintended path or correct the isolation requirement.",
    ),
    _rule(
        "OC6505",
        "connectivity-inconclusive",
        Severity.WARNING,
        "A connectivity path crosses syntax outside the bounded static subset.",
        "Simplify the cone, add supported structural RTL, or verify it with a native formal tool.",
    ),
    _rule(
        "OC6506",
        "connectivity-width-mismatch",
        Severity.ERROR,
        "Connectivity requirement endpoints have different bit widths.",
        "Select equal-width endpoints or split the requirement into explicit bit mappings.",
    ),
    _rule(
        "OC6507",
        "connectivity-bit-mapping-mismatch",
        Severity.ERROR,
        "A required static path has the wrong bit ordering.",
        "Correct the bus slice, concatenation, or identity/reverse transform declaration.",
    ),
    _rule(
        "OC6508",
        "connectivity-polarity-mismatch",
        Severity.ERROR,
        "A required static path has the wrong known inversion polarity.",
        "Correct the inversion or the declared connectivity transform.",
    ),
    _rule(
        "OC6509",
        "contradictory-connectivity-requirement",
        Severity.ERROR,
        "Connectivity intent contains duplicate or contradictory requirements.",
        "Give each requirement a unique ID and resolve conflicting endpoint expectations.",
    ),
    _rule(
        "OC9001",
